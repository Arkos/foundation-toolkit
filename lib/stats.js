function Stats() {
    ...
    this.recordOtherLeaders = function(table) {
        ...
        allTableLines = $("tr", table);

        for (var i = 1; i < allTableLines.length; i++) {
            var fields = $("td", allTableLines[i]);

            var planet;
            var cellChildren = fields[0].childNodes;
            var j;
            ...
            cellChildren = fields[index].childNodes;
            for (j = 0; j < cellChildren.length; j++) {
                if (cellChildren[j].nodeName.toLowerCase() == "img") {
                    ...
                    db.execute("insert into Leaders (name, position, account, color) values (?,?,?,?)", [leader, planet, utils.getOption("account"), color]);
                }
            }
        }
    };
    ...
    this.storeUsefulInformation = function() {
        ...
        utils.clearLeaders();
        for (var i = 0; i < allTablesHeaders.length; i++) {
            thisTableHeader = allTablesHeaders[i];
