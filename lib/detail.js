if (location.href.search("x=") != -1) {
    ...
    function Details() {
        ...
        this.linkifyLeaders = function() {
            var polTabChildren = $("#politique", realWindow.document)[0].childNodes;
        ...
        this.redesignPage = function() {
            ...
            for (var i = 0; i < divTroupesChildren.length; i++) {
                if (divTroupesChildren[i].nodeName.toLowerCase() == "img" &&
                    divTroupesChildren[i].src.match("croiseur") ) {
                    if (divTroupesChildren[i].src.match("x31") ) {
                        nbX31++;
                    }
                    else if (divTroupesChildren[i].src.match("x32") ) {
                        nbX32++;
                    }
                    else {
                        nbX27++;
            ...
            var nbX27String = document.createTextNode(nbX27 + " X27 ");
            var nbX31String = document.createTextNode(nbX31 + " X31 ");
            var nbX32String = document.createTextNode(nbX32 + " X32 ");
            ...
            if (nbX27 > 0) {
                divTroupes.insertBefore(nbX27String, divTroupes.lastChild);
                divTroupes.insertBefore(document.createElement("br"), divTroupes.lastChild);
                decalagePx += 15;
            }
            if (nbX31 > 0) {
                divTroupes.insertBefore(nbX31String, divTroupes.lastChild);
                divTroupes.insertBefore(document.createElement("br"), divTroupes.lastChild);
                decalagePx += 15;
            }
            if (nbX32 > 0) {
                divTroupes.insertBefore(nbX32String, divTroupes.lastChild);
                divTroupes.insertBefore(document.createElement("br"), divTroupes.lastChild);
                decalagePx += 15;
            }
            ...
            if (divOrdre.hasChildNodes()) {
                ordresPx = 0;
            }
            
            var newStyle = document.createElement("style");
            var newStyleContent = document.createTextNode("\n.ordre { visibility: visible } \n.troupes { left: 20px } \n.boutons { visibility: hidden } \n#ressources { top: " + (165 + decalagePx) + "px; height: 100px; visibility: visible } \n#politique { top: " + (470 + decalagePx - ordresPx) + "px; visibility: visible } \n#ordre { top: " + (345 + decalagePx) + "px; visibility: visible }\n");
        ...
        function findBestOffer() {
            var divResources = $("#ressources")[0];
            var divPol = $("#politique")[0];
